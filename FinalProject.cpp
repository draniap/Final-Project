...
#include<string.h> //memanggil library string untuk memanipulasi pengoprasian string dan array

// Deklarasi variabel yang digunakan input pesan
char plainInput[10000]; // Karakter untuk input pesan
char cipherText[20000]; // Karakter puntuk esan cipher
...
char ma[6][6]; // Untuk tabel array huruf dan angka
char barisKolom[6] = {'D', 'E', 'K', 'R', 'A', 'N'}; //Karakter baris dan kolom ciphertext
    ...
    mainMenu:
        ...
        switch(pilihan){ //membuat percabangan pilihan
            ...
            case '2': //mengevalusi kondisi (pilihan)
                dekripsi(); //memanggil fungsi dekripsi
                goto mainMenu;
                break; //untuk keluar dari struktur case 2
            case '3': //mengevalusi kondisi (pilihan)
                goto keluar;
                break; ////untuk keluar dari struktur case 3
            default:
...
// Fungsi yang digunakan untuk melakukan proses enkripsi
void enkripsi(){ //isi dari enkripsi yang akan dijalankan
    ...
    fflush(stdin); // digunakan untuk membersihkan sisa masukan dari input buffer 
    gets(plainInput);
    
    // Cari panjang kalimat
    panjangKalimat = strlen(plainInput); //strlen untuk menghitung jumlah karakter yang ada dalam suatu string
